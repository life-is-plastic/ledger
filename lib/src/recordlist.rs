...
impl Recordlist {
    ...
    pub fn spanned_interval(&self) -> Interval {
    ...
    pub fn slice_spanning_interval(&self, interval: Interval) -> &[Record] {
    ...
    fn index_of(&self, dt: Date, iid: usize) -> Option<usize> {
    ...
    pub fn get(&self, dt: Date, iid: usize) -> Option<&Record> {
    ...
    /// unmodified.
    pub fn remove(&mut self, dt: Date, iid: usize) -> Option<Record> {
        let i = self.index_of(dt, iid)?;
    ...
    pub fn iter_with_iid(&self) -> impl Iterator<Item = (usize, &Record)> {
        let mut iid = 0;
...
impl<'a> FromIterator<&'a Record> for Recordlist {
...
mod tests {
    ...
    fn test_sort_on_construction() {
        let rl = Recordlist::from_vec(vec![
            r#"{"d":"2015-03-30","c":"aaa","a":999}"#.parse::<Record>().unwrap(),
            r#"{"d":"2014-03-30","c":"bbb","a":888}"#.parse::<Record>().unwrap(),
            r#"{"d":"2016-03-30","c":"ccc","a":777}"#.parse::<Record>().unwrap(),
            r#"{"d":"2013-03-30","c":"ddd","a":666}"#.parse::<Record>().unwrap(),
        ]);
        let want_inner = vec![
            r#"{"d":"2013-03-30","c":"ddd","a":666}"#.parse::<Record>().unwrap(),
            r#"{"d":"2014-03-30","c":"bbb","a":888}"#.parse::<Record>().unwrap(),
    ...
    fn test_spanned_interval(#[case] rl: Recordlist, #[case] want: Interval) {
        assert_eq!(rl.spanned_interval(), want)
    }

    #[rstest]
    #[case("", Interval::EMPTY, "")]
    #[case(r#"{"d":"2015-03-30","c":"abc","a":111}"#, Interval::EMPTY, "")]
    ...
    fn test_slice_spanning_interval(
        ...
        assert_eq!(
            ...
            want.slice_spanning_interval(Interval::MAX)
        )
    }
    ...
    fn test_insert(#[case] mut rl: Recordlist, #[case] r: Record, #[case] want: Recordlist) {
        rl.insert(r);
        assert_eq!(rl, want)
    }
